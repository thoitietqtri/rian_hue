...
/** Tìm field thời gian và giá trị trong mảng object trả về từ API */
function normalizeApiRows(rows) {
  ...
  return rows
    .map((r) => {
      ...
      // đoán cột giá trị (số) đầu tiên khác cột thời gian
      const kVal =
...
/** Gộp nhiều series (mỗi series: [{time, value}], theo cột) thành bảng */
function buildTable(seriesByStation) {
  // Tập hợp tất cả mốc thời gian
...
/** Xuất Excel từ bảng (xlsx) */
function exportXLSX(filename, rows) {
  if (!rows.length) return;
  const ws = XLSX.utils.json_to_sheet(rows);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "solieu");
...
export default function App() {
  ...
  useEffect(() => {
    async function loadExcel() {
      try {
        ...
        // 3) Gợi ý mặc định: chọn 1–3 trạm đầu & ngày hôm qua → nay
        const now = new Date();
  ...
  // C) LẬP MAP mã trạm -> đối tượng trạm (để tra cứu nhanh)
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={() => exportCSV("solieu.csv", tableRows)}
                ...
                style={{
                  ...
                  background: "#fff",
                  cursor: tableRows.length ? "pointer" : "not-allowed",
                }}
              >
                Tải CSV
              </button>
              <button
                onClick={() => exportXLSX("solieu.xlsx", tableRows)}
                disabled={!tableRows.length}
                style={{
                  padding: "8px 12px",
                  borderRadius: 10,
